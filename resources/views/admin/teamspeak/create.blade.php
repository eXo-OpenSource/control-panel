                                ...
                                <form method="POST" action="{{ route('admin.users.teamspeak.store', [$user]) }}" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="custom-control custom-radio">
                                            <input type="radio" id="radioUser" name="type" value="1" class="custom-control-input{{ $errors->has('type') ? ' is-invalid' : '' }}" @if(old('type') === '1'){{ 'checked' }}@endif>
                                            <label class="custom-control-label" for="radioUser">{{ __('Benutzer') }}</label>
                                        </div>
                                        <div class="custom-control custom-radio">
                                            <input type="radio" id="radioMusicBot" name="type" value="2" class="custom-control-input{{ $errors->has('type') ? ' is-invalid' : '' }}" @if(old('type') === '2'){{ 'checked' }}@endif>
